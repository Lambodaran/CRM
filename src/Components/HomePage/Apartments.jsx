...
const Apartments = ({ selectedStateId, searchData, setSelectedStateId, setSearchData }) => {
  ...
  const parsePrice = (priceString) => {
    ...
    // Handle ranges (e.g., "50L-75L"), take the lower bound for filtering
    return value;
  };
  ...
  useEffect(() => {
    if (allProperties.length > 0) {
      ...
      if (searchData?.pricerange && typeof searchData.pricerange === "object" && searchData.pricerange.max !== undefined) {
        const maxPrice = parseFloat(searchData.pricerange.max);
        console.log("Applying price filter with maxPrice:", maxPrice);
        filtered = filtered.filter((property) => {
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          <p className="text-gray-500">
            {searchData?.pricerange?.max === 0 || searchData?.pricerange?.max === "0"
              ? "No properties found under Rs. 50 Lakh."
              : searchData?.pricerange?.max === 30000000 || searchData?.pricerange?.max === "30000000"
