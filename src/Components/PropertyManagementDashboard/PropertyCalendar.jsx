...
const PropertyCalendar = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || `Failed to save event: ${response.status}`
        );
      }
      ...
      >
        ...
        >
          <Box sx={{ flex: 1 }}>
            ...
            <Paper
              sx={{ p: { xs: 1, sm: 2 }, borderRadius: 2, height: "100%" }}
            >
              <FullCalendar
                ref={calendarRef}
                plugins={[
                      ...
                      >
                        ...
                        {event.targetUserId && (
                          ...
                          >
                            <UserAvatar>
                              {users
                                .find((user) => user._id === event.targetUserId)
                                ?.username?.charAt(0) || "U"}
                            </UserAvatar>
                            <Typography variant="caption">
                              {users.find(
                                (user) => user._id === event.targetUserId
                              )?.username || "Assigned"}
                            </Typography>
                          </Box>
                        )}
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              ...
              <Autocomplete
                ...
                onChange={(event, newValue) => {
                  ...
                  selectedEvent
                    ? setSelectedEvent({ ...selectedEvent, targetUserId: id })
                    : setNewEvent({ ...newEvent, targetUserId: id });
                }}
                renderInput={(params) => (
                  <TextField
                    {...params}
