    ...
    try {
      ...
      setFiles((prev) => {
        ...
        return newFiles.filter(
          (url) => typeof url === "string" || url instanceof File
        );
      });
    } catch (error) {
  ...
  const removeMedia = (index, type) => {
    switch (type) {
      ...
      case "video":
        ...
        break;
      case "unitPlans":
        setUnitPlans(unitPlans.filter((_, i) => i !== index));
        break;
      case "clubhousePlans":
        setClubhousePlans(clubhousePlans.filter((_, i) => i !== index));
        break;
      case "masterPlans":
        setMasterPlans(masterPlans.filter((_, i) => i !== index));
        break;
      case "elevationImages":
        setElevationImages(elevationImages.filter((_, i) => i !== index));
        break;
      case "interiorsImages":
        setInteriorsImages(interiorsImages.filter((_, i) => i !== index));
        break;
      case "amenitiesImages":
        setAmenitiesImages(amenitiesImages.filter((_, i) => i !== index));
        break;
      case "siteProgressImages":
        setSiteProgressImages(siteProgressImages.filter((_, i) => i !== index));
        break;
      case "walkthroughVideos":
        setWalkthroughVideos(walkthroughVideos.filter((_, i) => i !== index));
        break;
      default:
        break;
    }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const buildingData = {
        ...
        salientFeatures:
          data.salientFeatures?.split("\n").filter((line) => line.trim()) || [],
        projectOverview: data.projectOverview || "",
        locationAdvantages,
        configuration: {
          marketPrice: data.marketPrice || "",
          casagrandPrice: data.casagrandPrice || "",
        ...
        gallery: {
          ...
          interiors: interiorsImages.filter((p) => typeof p === "string"),
          amenities: amenitiesImages.filter((p) => typeof p === "string"),
          siteProgress: siteProgressImages.filter((p) => typeof p === "string"),
          walkthrough: walkthroughVideos.filter((v) => typeof v === "string"),
        },
      };
